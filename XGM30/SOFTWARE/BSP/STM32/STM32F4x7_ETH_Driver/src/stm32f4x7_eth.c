...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_CONTRL2);
    /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
    if(((RegValue & PHY_10M_FULL_DUPLEX_STATUS) != (uint32_t)RESET) || ((RegValue & PHY_100M_FULL_DUPLEX_STATUS) != (uint32_t)RESET))
    {
    ...
    if(((RegValue & PHY_10M_HALF_DUPLEX_STATUS) == PHY_10M_HALF_DUPLEX_STATUS) || ((RegValue & PHY_10M_FULL_DUPLEX_STATUS) == PHY_10M_FULL_DUPLEX_STATUS))
    {  
