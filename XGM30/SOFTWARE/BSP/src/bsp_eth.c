...
#define DP83848_PHY_ADDRESS       0x03 /* Relative to STM322xG-EVAL Board */
...
//  ETH_InitStructure.ETH_Mode = ETH_Mode_FullDuplex;   
  ...
  ETH_InitStructure.ETH_LoopbackMode = ETH_LoopbackMode_Disable;        //»·Â··µ»Ø
  ...
  /* if initial failed ,should be return*/
  if(!EthInitStatus)
  {
    return;
  }
...
uint32_t Eth_Link_PHYITConfig(uint16_t PHYAddress)
{
  ...
  /* Read MICR register */
  tmpreg = ETH_ReadPHYRegister(PHYAddress, PHY_KSZ8041_MICR);

  /* Enable output interrupt events to signal via the INT pin */
  tmpreg |= (uint32_t)PHY_KSZ8041_LINK_EN;
  if(!(ETH_WritePHYRegister(PHYAddress, PHY_KSZ8041_MICR, tmpreg)))
  {
...
void Eth_Link_ITHandler(uint16_t PHYAddress)
{
  ...
  /* Check whether the link interrupt has occurred or not */
  tmpreg = ETH_ReadPHYRegister(PHYAddress, PHY_KSZ8041_MICR);
  if(( tmpreg & PHY_LINK_INT_STATUS) != 0)
  {    
     /* Read PHY_SR*/
    tmpreg = ETH_ReadPHYRegister(PHYAddress, PHY_BSR);
    if(( tmpreg & PHY_KSZ8041_LINK_STATUS)!=0)
    {
       EthLinkStatus = 1;
